...
describe("SaudeVapor Governance test.", function () {
  it("Should go through the entire Proposal Lifecycle", async function () {
    ...
    console.log(`Uri: ${await svtoken.tokenURI(0)}`);
    ...
    // const svgovernor = await SVGovernor.deploy(
    //   svtoken.address,
    //   timelock.address
    // );
    ...
    // expect(executorole.events[0].args.account).to.equal(
    //   "0x0000000000000000000000000000000000000000"
    // );
    ...
    // const transferOwnershipTx = await svtoken.transferOwnership(
    //   timelock.address
    // );
    // const newTokenContractOwner = await transferOwnershipTx.wait(1);
    // expect(newTokenContractOwner.events[0].args.newOwner).to.equal(
    //   timelock.address
    // );
    // console.log(
    ...
    // const tokenLanguage = " Language Test 1 ";
    // const tokenTitle = " Title Test 1 ";
    // const tokenContent = " Content Test 1 ";
    // const timeStamp = `${new Date().getDate()} ${
    //   months[new Date().getMonth()]
    // } ${new Date().getFullYear()}`;

    // const transferCalldata = token.interface.encodeFunctionData("safeMint", [
    //   addressTo,
    //   tokenLanguage,
    //   tokenTitle,
    //   tokenContent,
    // ]);
    // const description = `Create a token to: ${addressTo}, in ${timeStamp}, with ${tokenLanguage} as default language. For contribuition of: ${tokenTitle}`;
    // const proposeTX = await svgovernor
    //   .connect(addr1)
    //   .propose([svtoken.address], [0], [transferCalldata], description);
    ...
    // console.log(
    //   `AgainstVotes: ${(
    //     await svgovernor.proposalVotes(proposalId)
    ...
    // );
    // console.log(
    //   `Quorum: ${await svgovernor.quorum(
    //     data.events[0].args.startBlock.toString()
    //   )}`
    // );

    // // TokenAddress, amount and transferCalldata from event
    // expect(await data.events[0].args.targets.toString()).to.equal(
    //   svtoken.address
    // );
    // expect(new BigNumber(await data.events[0].args[3]).c[0]).to.equal(0);
    // expect(await data.events[0].args.calldatas.toString()).to.equal(
    //   transferCalldata
    // );
    ...
    //   await data.events[0].args.description
    // );
    ...
    // expect((await executeTX.wait(1)).events[0].event).to.equal(
    //   "ProposalExecuted"
    // );
    ...
    // expect(
    //   await svtoken.balanceOf(await data.events[0].args.proposer)
    // ).to.equal(1);
    ...
    //   - Balance of Proposer: ${await svtoken.balanceOf(
    //     await data.events[0].args.proposer
    //   )}.`);
    // console.log(
    //   "-----------------------------------------------------------------"
    // );
  ...
  //   try {
  //     const setsvTOKENTx1 = await svtoken.safeMint(
  //       owner.address,
  //       " String Test 2 ",
  //       " String Test 2 ",
  //       " String Test 2 "
  ...
  //   // Cast a Vote
  //   console.log(
  //     "---------Cast vote from an account that does not have a Token"
  //   );
  //   console.log(`Account to vote: ${addr2.address}`);
  //   const voteWay = 1; // 0 = Against, 1 = For, 2 = Abstain
  //   const voteTx = await svgovernor
  //     .connect(addr2)
  //     .castVote(proposalId, voteWay);
  //   await voteTx.wait(1);
  //   const hasVoted = await svgovernor
  //     .connect(addr2)
  //     .hasVoted(proposalId, addr2.address);
  //   expect(await hasVoted).to.equal(true);
  //   console.log(`Vote done: ${await hasVoted}`);
  //   expect((await svgovernor.proposalVotes(proposalId)).toString()).to.equal(
  //     "0,0,0"
  //   );
  //   console.log(
  //     `AgainstVotes counted: ${(
  //       await svgovernor.proposalVotes(proposalId)
  //     ).againstVotes.toString()}`
  //   );
  //   console.log(
