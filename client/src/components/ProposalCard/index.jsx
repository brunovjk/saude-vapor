...
function ProposalCard({ data }) {
  ...
  const [linkToToken, setLinkToToken] = useState(
    "/blockchain/ProposalInfo/:proposalId"
  );
  ...
  useEffect(() => {
    try {
      ...
      if (data.proposalState === 0) {
        ...
        setChiplabel(t("ProposalCard.success"));
      } else if (data.proposalState === 1) {
        selChipColor("primary");
        setChiplabel(t("ProposalCard.ongoing"));
      } else {
        selChipColor("error");
        setChiplabel(t("ProposalCard.rejected"));
      }
    } catch (error) {
